    ...
    return switch(term) {
      case TermNode.Assertion(assertion):
        function(state:MatcherState, continuation:Continuation):MatcherResult {
          if (term == null) {
            // TODO: Remove this check, it is only there due to an issue in Open Flash's parser/emitter
            return MatcherResult.Failure;
          }
      ...
      case TermNode.Atom(atom):
        RegExpMatcher.evaluateAtom(atom);
      case TermNode.QuantifiedAtom(atom, quantifier, captureStart, captureEnd):
        RegExpMatcher.evaluateQuantifiedAtom(atom, quantifier, captureStart, captureEnd);
    }
  }
